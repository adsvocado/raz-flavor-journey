          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="hidden md:flex items-center space-x-8">
              ...
              >
                ...
                {/* PANEL DEL DROPDOWN */}
                <div
                  className={`absolute top-full left-0 mt-2 transform transition-all duration-200 ${
                    isProductsOpen
                      ? "opacity-100 translate-y-0 pointer-events-auto"
                      : "opacity-0 -translate-y-1 pointer-events-none"
                  }`}
                >
                  <div className="glass rounded-2xl py-2 shadow-glass min-w-[160px]">
                    <ul className="flex flex-col">
                      {productLinks.map((product) => (
                        <li key={product.href}>
                          <Link
                            to={product.href}
                            className="block px-4 py-2 text-sm font-poppins text-foreground hover:bg-white/10 rounded-xl"
                          >
                            {product.name}
                          </Link>
                        </li>
